                    ...
                    <div class="col-12">
                        @isset($membership)
                            @if($membership->membership_status == 'INACTIVE')
                                <p class="text-danger">आपकी सदस्यता योजना व्यवस्थापक द्वारा अनुमोदित नहीं है कृपया
                                    व्यवस्थापक से संपर्क करें।</p>
                            ...
                            @else
                                <p class="text-danger">आपकी कोई सक्रिय सदस्यता योजना नहीं है, कृपया सदस्यता फॉर्म
                                    भरें।</p>
                            @endif
                        @else
...
@section('scripts')
    ...
    <script>
        ...
        const storeMembership = (razorpay_payment_id = '') => {
            $.post("{{ route('franchisee.store.membership') }}", $('#membershipForm').serialize() + '&razorpay_payment_id='+razorpay_payment_id, result => {
                if (result.errors) {
                    ...
                    if(result.status){
                        window.location = "{{ route('franchisee.dashboard') }}"
                    }else{
