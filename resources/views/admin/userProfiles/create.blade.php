...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.create') }} {{ trans('cruds.userProfile.title_singular') }}
        </div>
            ...
            <form method="POST" action="{{ route("admin.user-profiles.store") }}" enctype="multipart/form-data">
                ...
                <div class="row">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}"
                                ...
                                @foreach($users as $id => $user)
                                    <option
                                        value="{{ $id }}" {{ old('user_id', Request::input('user_id')) == $id ? 'selected' : '' }}>{{ $user }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('user'))
                    ...
                    <div class="col-6">
                        <div class="form-group">
                            ...
                            <input class="form-control {{ $errors->has('mobile') ? 'is-invalid' : '' }}" type="text"
                                   name="mobile" id="primary_contact" value="{{ old('mobile', '') }}">
                    ...
                    <div class="col-6">
                        <div class="form-group">
                            <label
                                for="secondary_contact">{{ trans('cruds.userProfile.fields.secondary_mobile') }}</label>
                            <input class="form-control {{ $errors->has('secondary_mobile') ? 'is-invalid' : '' }}"
                                   type="text" name="secondary_mobile" id="secondary_contact"
                            ...
                            @endif
                            <span
                                class="help-block">{{ trans('cruds.userProfile.fields.secondary_mobile_helper') }}</span>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="form-group">
                            <label
                                for="farming_land_area">{{ trans('cruds.userProfile.fields.farming_land_area') }}</label>
                            <input class="form-control {{ $errors->has('agricultural_land') ? 'is-invalid' : '' }}"
                                   type="number" name="agricultural_land" id="farming_land_area"
                                   value="{{ old('agricultural_land', '') }}" step="0.01">
                            ...
                            @endif
                            <span
                                class="help-block">{{ trans('cruds.userProfile.fields.farming_land_area_helper') }}</span>
                        </div>
                    </div>
                </div>
                <div class="form-group">
