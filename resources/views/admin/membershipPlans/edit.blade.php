...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.edit') }} {{ trans('cruds.membershipPlan.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route("admin.membership-plans.update", $membershipPlan) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-6">
                        <div class="form-group">
                            ...
                            <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="plan_type" id="plan_type" value="{{ old('plan_type', $membershipPlan->plan_type?? '') }}" required>
                    ...
                    <div class="col-6">
                        <div class="form-group">
                            ...
                            <input class="form-control {{ $errors->has('fees') ? 'is-invalid' : '' }}" type="number" name="fees" id="fees" value="{{ old('fees', $membershipPlan->fees, '') }}" required>
                            @if($errors->has('fees'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('fees') }}
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control {{ $errors->has('member_type') ? 'is-invalid' : '' }}" name="member_type" id="member_type" required>
                                ...
                                @foreach(App\Models\MembershipPlan::MEMBER_TYPES_RADIO as $key => $label)
                                    <option value="{{ $key }}" {{ old('member_type', $membershipPlan->member_type ?? '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                                ...
                                @foreach(App\Models\MembershipPlan::STATUS_RADIO as $key => $label)
                                    <option value="{{ $key }}" {{ old('status', $membershipPlan->status ?? '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
