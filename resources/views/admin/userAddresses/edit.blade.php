                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2 {{ $errors->has('user_id') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                            @foreach($users as $id => $user)
                                <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $userAddress->user_id ?? '') == $id ? 'selected' : '' }}>{{ $user }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2 {{ $errors->has('state_id') ? 'is-invalid' : '' }}" name="state_id" id="state_id" required>
                            @foreach($states as $id => $state)
                                <option value="{{ $id }}" {{ old('state_id', $userAddress->state_id ?? '') == $id ? 'selected' : '' }}>{{ $state }}</option>
                            @endforeach
                        </select>
                ...
                <div class="col-6">
                    <div class="form-group mb-3">
                        ...
                        <input class="form-control {{ $errors->has('pincode') ? 'is-invalid' : '' }}"
                               ...
                               value="{{ old('pincode', $userAddress->pincode ?? '') }}" required>
...
@section('scripts')
    <script>
        $(document).ready(function () {
            ...
            let state = "{{ old('state_id', $userAddress->state_id ?? '') }}";
            let district = "{{ old('district_id', $userAddress->district_id ?? '') }}";
