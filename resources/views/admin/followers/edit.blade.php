            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $follower->user->id ?? '') == $id ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('follow') ? 'is-invalid' : '' }}" name="follow_id" id="follow_id" required>
                    @foreach($follows as $id => $follow)
                        <option value="{{ $id }}" {{ (old('follow_id') ? old('follow_id') : $follower->follow->id ?? '') == $id ? 'selected' : '' }}>{{ $follow }}</option>
                    @endforeach
                </select>
