...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="post" action="{{ route('admin.help-center-memberships.store') }}" enctype="multipart/form-data">
                @csrf
                <div class="row">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2 {{ $errors->has('help_center') ? 'is-invalid' : '' }}"
                                ...
                                @foreach($helpCenters as $id => $helpCenter)
                                    <option
                                        value="{{ $id }}" {{ old('member_id') == $id ? 'selected' : '' }}>{{ $helpCenter }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="plan_type" id="plan_type" required>
                                ...
                                @foreach($membershipPlans as $plan)
                                    <option
                                        value="{{ $plan->id }}" {{ old('plan_type') == $plan->plan_type ? 'selected' : '' }}>{{ $plan->plan_type }}
                                        - {{ $plan->fees }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="payment_method" id="payment_method" required>
                                ...
                                @foreach(App\Models\HelpCenterProfile::PAYMENT_METHOD_RADIO as $key => $label)
                                    <option
                                        value="{{ $key }}" {{ old('payment_method') ?? '' == $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="membership_status" id="membership_status" required>
                                ...
                                @foreach(App\Models\Membership::MEMBERSHIP_STATUS_RADIO as $key => $label)
                                    <option
                                        value="{{ $key }}" {{ old('membership_status') ?? '' == $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
