...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="post" action="{{ route('admin.franchisee-memberships.update', $membership) }}" enctype="multipart/form-data">
                @csrf
                @method('put')
                <div class="row">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2 {{ $errors->has('help_center') ? 'is-invalid' : '' }}"
                                ...
                                @foreach($franchisees as $id => $franchisee)
                                    <option
                                        value="{{ $id }}" {{ old('member_id', $membership->member_id ?? '') == $id ? 'selected' : '' }}>{{ $franchisee }}</option>
                                @endforeach
                            </select>
