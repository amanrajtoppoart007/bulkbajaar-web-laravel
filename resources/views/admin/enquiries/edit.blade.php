    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.enquiries.update", [$enquiry->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('mobile') ? 'is-invalid' : '' }}" type="text" name="mobile" id="mobile" value="{{ old('mobile', $enquiry->mobile) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('subject') ? 'is-invalid' : '' }}" type="text" name="subject" id="subject" value="{{ old('subject', $enquiry->subject) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('attendant') ? 'is-invalid' : '' }}" name="attendant_id" id="attendant_id">
                    @foreach($attendants as $id => $attendant)
                        <option value="{{ $id }}" {{ (old('attendant_id') ? old('attendant_id') : $enquiry->attendant->id ?? '') == $id ? 'selected' : '' }}>{{ $attendant }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="email" name="email" id="email" value="{{ old('email', $enquiry->email) }}">
                @if($errors->has('email'))
                    <div class="invalid-feedback">
                        {{ $errors->first('email') }}
                    </div>
