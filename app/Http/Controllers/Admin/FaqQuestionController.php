...
class FaqQuestionController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $query = FaqQuestion::with(['category', 'created_by'])->select(sprintf('%s.*', (new FaqQuestion)->table));
        ...
        return view('admin.faqQuestions.index', compact('faq_categories', 'users'));
    }
    ...
    public function edit(FaqQuestion $faqQuestion)
    {
        ...
        $faqQuestion->load('category', 'created_by');
